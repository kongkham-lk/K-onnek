...
const iconMap = { linkedin: BriefcaseBusiness, github: GitBranch, file: FileText, code: Code2, mail: Mail, phone: Phone }

function ThemeToggle({ theme, onToggle }) {
  const isDark = theme === 'dark'
  return (
    <button className="icon-button theme-toggle" type="button" onClick={onToggle} aria-label={`Switch to ${isDark ? 'light' : 'dark'} theme`}>
...
function Profile() {
  ...
  return (
    <header className="profile">
      <div className="photo-frame"><img src={profile.photo} alt={profile.photoAlt} /></div>
      <h1>{profile.name}</h1>
      <p className="experience">
        {profile.previous.map((phrase, index) => (
          <span className="experience-phrase" key={phrase}>
            {index > 0 && <span className="experience-separator">•</span>}
            <span className="experience-nowrap">{phrase}</span>
          </span>
        ))}
      </p>
...
function LinkCard({ link }) {
  ...
  const attributes = link.external ? { target: '_blank', rel: 'noopener noreferrer' } : {}
  return (
    <a className="link-card" href={link.url} {...attributes}>
      <span className="link-icon"><Icon size={19} strokeWidth={1.8} /></span>
      <span className="link-copy"><strong>{link.label}</strong><span>{link.description}</span></span>
      <ArrowUpRight className="link-arrow" size={18} strokeWidth={1.7} />
    </a>
  )
}
...
function QrCodeModal({ onClose }) {
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(qrUrl, { width: 480, margin: 2, color: { dark: '#11120f', light: '#ffffff' } }).then(setQrData)
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <div className="qr-dialog" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="qr-title">
        <button className="icon-button modal-close" type="button" onClick={onClose} ref={closeButtonRef} aria-label="Close QR code dialog"><X size={19} /></button>
        <div className="dialog-kicker"><QrCode size={16} /> Share this card</div>
        <h2 id="qr-title">Scan to connect</h2>
...
function App() {
  const [theme, setTheme] = useState(() => localStorage.getItem('contact-card-theme') || siteConfig.appearance.defaultTheme)
  const [showQr, setShowQr] = useState(false)

  useEffect(() => {
    document.documentElement.dataset.theme = theme
    localStorage.setItem('contact-card-theme', theme)
